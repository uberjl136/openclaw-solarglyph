...
<AccordionGroup>
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    Legacy outbound and session delivery queues are imported from `delivery-queue/` and `session-delivery-queue/` into SQLite. At this one-time cutover, pending entries whose original enqueue time is **72 hours old or older** are preserved without automatic delivery. Missing, invalid, or future enqueue times also require manual review. A recent retry or file modification does not renew an old message. Normal SQLite queue delivery and retry policies are unchanged.
