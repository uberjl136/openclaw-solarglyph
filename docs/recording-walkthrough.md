...
### 最省事的做法：双击现成的"分步执行"文件（推荐）

手打命令容易出错，也容易在粘贴时被 PowerShell 截断（表现为突然冒出一行红色
`无法将"removed"项识别为 cmdlet`）。为此准备了两套现成文件，**不用打任何字**：

**方式一：桌面上已经放了 4 个文件，直接双击**

| 双击这个 | 相当于要录的镜头 |
| --- | --- |
| 桌面 → `录视频-镜头1.bat` | 镜头 1：卸载技能 → 列出上游能力（**21/57**） |
| 桌面 → `录视频-镜头2.bat` | 镜头 2：装回技能 → 显示已识别（**22/58**） |
| 桌面 → `录视频-镜头3.bat` | 镜头 3：跑仿真 → 出结果（核心演示） |
| 桌面 → `录视频-镜头4.bat` | 镜头 4：自检 + 提交历史 |

**方式二：用仓库里的脚本**（剧本随仓库一起发布，换台电脑也能用）

```
work\solarglyph-skill\scripts\recording\run-record-step.bat 1
```

打开方法：在文件管理器进到 `work\solarglyph-skill\scripts\recording`，
**在地址栏输入 `cmd` 回车**，再输入 `run-record-step.bat 1` 回车。

四种编号分别对应四个镜头。这些文件会**自动设好所有必需的环境变量**，
不需要你在窗口里预先输任何东西；里面的命令与下面各镜头列出的完全一致，旁白照念即可。

> 若你更喜欢自己敲命令，就继续按下面各镜头的手打版本操作；
> 但只要记住"**一次只粘一行**"，就不会出现红色报错。
